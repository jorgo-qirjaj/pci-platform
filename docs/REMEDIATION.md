...
- ⬜ **C4 · No object-level authorization (IDOR) + enumerable IDs** · Security/Privacy · `cases.ts:75/82/100/109`, `store.ts:78-85` — any authenticated user may read/score/finalize **any** case, and accessions are sequential (`PCI-2026-00001…`). **Impact:** one token enumerates and mutates all clinical records. **Fix:** per-user/lab ownership checks + non-guessable IDs (UUID) + tenant scoping. **Effort: L**
