...
| **H9 — Tests + CI** | 🟡 **Partial** | Node `node:test` + supertest harness with **33 tests** across authz, scoring, validation, audit, auth, annotations, and the upload proxy; CI gates typecheck + tests. Remaining: **lint config** (open decision) + React-screen tests. |
...
- 🟡 **H9 · No tests, no CI, no lint** · Quality/Safety net · repo-wide. **Progress:** a test harness now exists — Node `node:test` + Supertest run via tsx (deliberately no vitest/vite/esbuild, keeping `npm audit` at 0), with the C4 authorization suite as the first coverage. A CI workflow gating typecheck + tests is prepared (`.github/workflows/ci.yml`; needs a workflow-scoped push). **Coverage broadened (ticket 602):** 33 tests now span authz, scoring integrity, request/JWT validation, audit log, auth (`/me` + rate-limit), annotations, and the upload proxy. **Still open:** a **lint config** (ESLint — open decision on strictness) and React-screen tests. **Effort: M–L**
...
- 🟡 **M6 · Accessibility gaps** · **Fix shipped (structural, ticket 503):** case rows are keyboard-operable + labelled; `NewCaseModal` is a proper dialog (focus trap, Esc, `role="dialog"`); the `TopBar` account menu is semantic (`button` + `aria-haspopup`/`aria-expanded`, `role="menu"`/`menuitem`, Esc). Verified in-browser. **Remaining:** WCAG contrast audit + a full axe-core pass. **Effort: M**
